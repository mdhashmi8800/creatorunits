...
export default function PngToWebp() {
  ...
  useEffect(() => {
    const processNext = async () => {
      ...
      try {
        ...
        // WebP supports transparency, conversion works natively
        const result = await convertImage(imageSrc, { outputType: "image/webp", quality: 85 });
  ...
  const handleDownload = (item: ImageItem) => {
    ...
    showToast(`Downloaded WebP for ${item.file.name}`, "success");
  };
  ...
  const handleDownloadAll = () => {
    ...
    showToast(`Downloading all ${successItems.length} WebP images...`, "success");
  };
    ...
    <div className="card">
      {items.length === 0 ? (
        <FileUpload
          ...
          accept="image/png"
          maxSizeMB={15}
          label="Upload PNG images to convert"
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex justify-between items-center flex-wrap gap-2">
              <div>
                ...
                <p className="text-muted" style={{ margin: 0, fontSize: "0.85rem" }}>
                  Converted {successCount} of {items.length} PNG files to WebP.
                </p>
              </div>
              {pendingCount > 0 && (
                ...
                <div className="flex items-center gap-3" style={{ minWidth: "200px", flex: 1 }}>
                  ...
                  >
                    {item.imageSrc ? (
                      ...
                      <span style={{ fontSize: "0.85rem", color: "white" }}>PNG</span>
                    )}
                  </div>
          ...
          <div className="flex-actions">
            ...
            >
              📥 Download All WebPs ({successCount})
            </button>
          </div>
        </div>
      )}
